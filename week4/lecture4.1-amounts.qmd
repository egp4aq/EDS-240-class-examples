...
jobs_clean %>%
  ...
  mutate(occupation = fct_reorder(occupation, total_earnings)) %>% # lets us reorder factor levels (occupation) based on another variable in our df (total_earnings) --> this allows us to order the plot from highest to lowest earnings
  ggplot(aes(x = occupation, y = total_earnings)) +
  geom_col() +
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k")) + # for money!
  geom_text(aes(label = dollar(total_earnings)),
            hjust = 1.2, color = "white") + # hjust adjusts horizontal justification (just try and adjust accordingly)
  coord_flip() # good to flip coords when the axis labels are too long
```
...
jobs_clean |> 
  ...
  ggplot(aes(x = occupation, y = total_earnings)) +
  geom_point() +
  geom_linerange(aes(ymin = 0, ymax = total_earnings)) + # need to define beginning and end point for line (it inherited the x axis position from our global aesthetics)
  coord_flip() + # flip axes
  scale_y_continuous(labels = label_currency(scale = 0.001, suffix = "k")) + # we use y because techinically we mapped total_earnings to our y axis in the universal
  geom_text(aes(label = dollar(total_earnings)),
            hjust = -0.2, color = "darkblue") +
  ylim(0, 250000) # adjust axis so that we can see the full label (could do this adjustment in scale_y_continuous function)
```
...
jobs_clean |> 
  ...
  mutate(sex = str_remove(string = group, pattern = "total_earnings_")) %>% # snip off the beginning and only leave male / female
